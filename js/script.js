...
const Pagestext = [
    {title:'No.1　《頭の中のゴミを捨てる》' ,text:'「頭の中のゴミの名前」と書かれている枠をクリックして、捨てたい頭の中のゴミの名前を入力します。入力した後に「捨てる」ボタンを押すと、【ゴミリスト】の下にゴミが追加されます。'},
    {title:'No.2　《ゴミをゴミ箱に捨てる》' ,text:'「捨てるゴミ箱を変える」ボタンを押すと、【ゴミ箱リスト】にあるゴミ箱の名前が右に書かれたチェックボックスが出てきます。捨てたい先のゴミ箱の名前が付いたチェックボックスをクリックしてチェックした後に「ゴミ箱に捨てる」ボタンを押すと、そのゴミ箱の中にゴミが追加されます。「捨てるゴミ箱を変えない」ボタンを押すと、そのまま「捨てるゴミ箱を変える」が出ていた状態に戻ります。'},
    {title:'No.3　《ゴミの名前・説明編集　削除》' ,text:'「このゴミを消滅させる」ボタンを押すと、そのゴミが消滅します。\n「名前・説明を変える」ボタンを押すと、「新しい名前を入力してください。」と書かれた欄が出てきます。そこに新しい名前や説明文を入力したらOKボタンを押してください。名前や説明文が変更されます。'},
    {title:'No.4　《ゴミ箱の配置》' ,text:'「ゴミ箱の名前」と書かれている枠をクリックして、ゴミ箱の名前を入力します。入力した後に「ゴミ箱を追加する」ボタンを押すと、【ゴミ箱リスト】の下にゴミ箱が追加されます。'},
    {title:'No.5　《ゴミ箱の名前・説明編集　削除》' ,text:'「このゴミ箱を削除する」ボタンを押すと、そのゴミ箱が削除されます。\n「名前・説明を変える」ボタンを押すと、「新しい名前を入力してください。」等と書かれた欄が出てきます。そこに新しい名前や説明文を入力したらOKボタンを押してください。名前や説明文が変更されます。'},
    {title:'No.6　《リサイクルで新しいものを作る》' ,text:'「このゴミ箱からリサイクル」ボタンを押すと、「このゴミからリサイクルする」と右に書かれているチェックボックスにチェックされているゴミを用いて新たなものを作り出します。その新たなものの名前とそれについての説明を入力してください。（説明の有無は任意です。）入力してそれぞれOKボタンを押すと、それが【リサイクルによって生まれたものリスト】の下に追加されます。そのゴミ箱に入っているそのゴミ箱にゴミが入っていない状態ではリサイクルできません。'},
    // {title:'No.7　《作ったものの名前・説明編集　削除》' ,text:'「このゴミ箱を削除する」ボタンを押すと、そのゴミ箱が削除されます。\n「名前・説明を変える」ボタンを押すと、「新しい名前を入力してください。」等と書かれた欄が出てきます。そこに新しい名前や説明文を入力したらOKボタンを押してください。名前や説明文が変更されます。'},
    {title:'No.8　《説明書の読み方》' ,text:''},
];
const PagesImg = [,,];
const PagesImgAlt = ['1の写真','2の写真','3の写真'];
let NowPage = 0;
